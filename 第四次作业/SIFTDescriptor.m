...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        grad_mag{scale}= sqrt(img_dy.^2+img_dx.^2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        feature = [];
       cellmag=mat2cell( patch_mag,ones(patch_size/num_histograms,1)*num_histograms,ones(patch_size/num_histograms,1)*num_histograms);
       celltheta=mat2cell( patch_theta,ones(patch_size/num_histograms,1)*num_histograms,ones(patch_size/num_histograms,1)*num_histograms);

for i=1:num_histograms
    for j=1:num_histograms
       [histogram, angles] = ComputeGradientHistogram( num_angles, cellmag{i,j}, celltheta{i,j}); 
       feature=[feature histogram];
    end
end
...
length=numel(gradient_angles);
for i=1:length
    if isnan(gradient_angles(i))==1  %0/0会得到nan，如果像素是nan，重设为0
                   gradient_angles(i)=0;
    end
    ang=gradient_angles(i);
                        
                ang=ang+0.0000001;          %防止ang为0
               histogram(ceil(ang/angle_step))=histogram(ceil(ang/angle_step))+ gradient_magnitudes(i); 
